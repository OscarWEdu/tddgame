...
public interface ITurnsRepository
{
  ...
  Task<bool> ChangeCurrentTurnPhaseAsync(Guid gameSessionId, string newPhase, CancellationToken ct); // Changes the phase of the active turn.
  Task<bool> SetTurnStatusAsync(int turnId, string status, CancellationToken ct); // Changes the status of a turn row.
